...
export default function TopBar() {
  const { selectedYear, activeRace, selectedSession, setCommandPalette } = useSettingsStore();
  ...
  return (
    <header className="h-14 flex items-center justify-between px-6 border-b border-white/10 bg-black/40 backdrop-blur-md flex-shrink-0 z-40 sticky top-0">
      <div className="flex items-center text-sm text-apex-muted gap-2">
        <span className="font-semibold text-white/90">{selectedYear} Season</span>
        <span className="text-white/20">•</span>
        <span className="text-white/70">{activeRace?.country || "Global"}</span>
        <span className="text-white/20">•</span>
